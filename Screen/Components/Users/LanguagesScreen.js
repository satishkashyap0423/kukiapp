...
class LanguagesScreen extends Component {
  ...
  openStatusModal() { 
    ...
    if(this.state.selectedLetter.length < 1){
      this.setState({errMsg:AlertMessages.languageErr});
  ...
  getLanguageList = async () => {
    ...
    axios.post(Api.apiUrl+'/get-language', data, headers)
    .then(res => {
      this.state.loading = false;
      this.setState({itemsData:res.data.data.sort((a, b) => a.name.localeCompare(b.name))}); 
    })
    .catch(error => {
  ...
  getListViewItem = (item) => {  
    ...
    if(this.state.selectedLetter.length > 5){
      this.setState({errMsg:AlertMessages.maxlanguageErr});
      this.setState({ishowMOdal:true});
  ...
  //return;
  preVvalue.languages = this.state.selectedLetter.toString();
  ...
  {ishowMOdal && <UseAlertModal message={errMsg} parentCallback = {this.handleCallback} /> }
   <View style={styles.mainBody}>
      ...
      <View style={styles.SectionHeadStyle}>
         <Text style={styles.SectionHedText}>Select Languages You Know</Text>
      </View>
      <View style={styles.ContentHeadStyle}>
         <Text style={styles.SectionContText}>Choose maximum 5 languages</Text>
      </View>
   ...
   <View style = {styles.Mdcontainer}>  
        <Modal            
          ...
          {/*All views of Modal*/}  
              ...
              <Text style = {styles.modalTitle}>{AlertMessages.filedLanguageMsg}</Text> 
