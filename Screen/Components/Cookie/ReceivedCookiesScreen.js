...
class ReceivedCookiesScreen extends Component {
  ...
  async getCookieData() {
    ...
    console.log(data);
    axios.post(Api.apiUrl + '/received-cookie-record', data, headers)
      .then(res => {
  ...
  openCookie = async (val, fuser, isFor) => {
    ...
    axios.post(Api.apiUrl + '/open-cookie', data, headers)
      .then(res => {
        this.setState({ loading: false });
        if (res.data.status == 'true') {
          this.props.navigation.push('CookiesDetail', { user_id: fuser, isFrom: 2 });
  ...
  render() {
    ...
    if (search_data.length > 0) {
      ...
      receiveContentData.push(<Text style={styles.SectionNoRecordStyle} key='c5'>{AlertMessages.receivedErrmsg}</Text>);
    }
    return <View style={{ flex: 1, height: '100%', backgroundColor: '#fff' }}><Loader loading={loading} /><ScrollView>
      {ishowMOdal && <UseAlertModal message={errMsg} parentCallback={this.handleCallback} />}<View style={styles.mainBody}>
        <View style={styles.topheadSection}>
          ...
          <Text style={styles.SectionHeadStyle}>Received Kukies</Text>
