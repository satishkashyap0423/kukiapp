 ...
 constructor(props) {
   ...
   sendCookie = async (val,isSe,i,isNewcookieStat,fName,lName) => {
    ...
    this.setState({loading:true});
      ...
      .then(res => {
        ...
        this.setState({ishowMOdal:true});
         let sData = this.props.route.params.searchFellowData;
  ...
  for(let i =0; i<search_data.length;i++){
    let isSameguru = false;
      if(userDetail.guru_id == search_data[i].guru_id){
         isSameguru = true;
      }
    let userImg = [];
   ...
   if(userRating == null){
    ...
    if(search_data[i].isCookieStat == '1'){
      ckImg.push(<TouchableOpacity
         key="dds"
                  onPress={this.sendCookie.bind(this,search_data[i].user_id,1,i,search_data[i].isNewcookieStat,search_data[i].first_name, search_data[i].last_name)} 
      ...
      ckImg.push(<TouchableOpacity
       key="ddds"
                  onPress={this.sendCookie.bind(this,search_data[i].user_id,0,i,search_data[i].isNewcookieStat,search_data[i].first_name, search_data[i].last_name)} 
...
if(search_data[i].is_same_blocked == '1' && search_data[i].is_blocked == '0'){
      ckImg = [];
      ckImg.push(<TouchableOpacity
      key={search_data[i].user_id}
    ...
    searchContentData.push(
              <TouchableOpacity
              activeOpacity={0.6}
              style={[styles.buttonStyle, i == 0 ? styles.mainInnersectionNob : styles.mainInnersection]} key={i}
              onPress={this.getuserDetail.bind(this,search_data[i].user_id,1)} >{userImg}
                <View style={styles.searchMainContentsection} >
                  <View style={styles.searchContentsection}>
                    ...
                    <View style={styles.samegurusect}><Text style={styles.searchContentTextBold}>{search_data[i].first_name} {search_data[i].last_name}, <Feather 
                  name={'star'}
                  size={16} 
                  color={AppStyle.appIconColor}
                   
                /> {uRate}</Text>{isSameguru && <View style={styles.sameguruContOut}><Text style={styles.searchContentText}>, </Text><Image
      
                  source={require('../../../assets/images/icons/gurubhai.jpg')}
                  style={styles.sameguruCont}
                /></View> }</View>
                    <Text style={styles.searchContentText}>{search_data[i].gender}, {search_data[i].age_group}, {search_data[i].city_data.name}, {(search_data[i].state_data.state_abbrivation != '' ? search_data[i].state_data.state_abbrivation : search_data[i].state_data.name)}</Text>
  ...
  searchContentData.push(<Text style={styles.SectionNoRecordStyle} key='5'>{AlertMessages.searchErrmsg}</Text>);
}
